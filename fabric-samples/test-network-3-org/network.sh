...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-RBI.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-HDFC.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-SBI.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  infoln "Generating CCP files for RBI, HDFC and SBI"
